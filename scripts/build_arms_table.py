...
def main() -> int:
    ...
    L.append("`— (không có TerminalAction)` = LLM-only không phát ra phán quyết nào, nên")
    L.append("*\"nó có từ chối không\"* phải đọc văn bản. Đoán bằng regex là dựng một phép")
    L.append("đo giả, nên ô để trống.\n")
    L.append("⚠️ **Hai ô trống này là vĩnh viễn, không phải việc còn treo.** Bản trước")
    L.append("của dòng này nói ngược lại — nó chỉ sang việc (3) của Tuần 6 như thứ sẽ")
    L.append("điền chúng vào. Việc (3) **đã xong** (2026-09-14), và ô vẫn trống: thứ nó")
    L.append("trả về đo *mức bám ngữ cảnh của câu trả lời*, một thang đo khác hẳn")
    L.append("leakage và coverage. Trộn hai thang đo vào một hàng bảng là đúng cái")
    L.append("`docs/ragas.md` dành hẳn một cảnh báo riêng để cấm.")
    L.append("So sánh thật giữa LLM-only và corrective nằm ở **bảng ghép cặp** của")
    L.append("`docs/ragas.md` — đọc ở đó, đừng đọc ở hàng này.\n")
